...
<%
  ...
  try {
	 ...
	 String sql="select * from attackdefense inner join img1 on img1.img_no=attackdefense.magic_ip inner join love1 on love_no=img1.img_no where Ename = '"+id +"'";
...
$(function(){ 
    $('#id02').on('click',function(){  
              //alert("데이터 전송~ ^^*"); 
       $.ajax({ 
               url:'mainlove.jsp', //가져오고자하는 서버페이지 주소를 넣는다. 
...
<body onload="document.body.scrollTop=document.cookie" onunload="document.cookie=document.body.scrollTop">
  ...
  <div class="modal-header">
        ...
        <center><img src="img/spell/attackdefense/<%=rs.getString("img_src") %>.<%=rs.getString("img_file") %>" style="width:300px;height:120px;"></center>
        </div>
...
<br>
  ...
  int hit = rs.getInt("Readcount");
	hit++;
  sql = "UPDATE attackdefense SET Readcount=" + hit + " where Ename='" +rs.getString("Ename")+"'"; 
