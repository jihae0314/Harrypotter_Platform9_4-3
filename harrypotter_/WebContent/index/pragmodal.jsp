...
<%
  ...
  try {
	 ...
	 String sql="select * from pragmatic inner join img2 on img2.img_no=pragmatic.magic_ip inner join love2 on love_no=img2.img_no where Ename = '"+id +"'";
...
$(function(){ 
    ...
    $('#id02').on('click',function(){  
              //alert("데이터 전송~ ^^*"); 
       $.ajax({ 
               url:'praglove.jsp', //가져오고자하는 서버페이지 주소를 넣는다. 
...
<body onload="document.body.scrollTop=document.cookie" onunload="document.cookie=document.body.scrollTop">
  ...
  <div class="modal-header">
        ...
        <center><img src="img/spell/pragmatic/<%=rs.getString("img_src") %>.<%=rs.getString("img_file") %>" style="width:300px;height:120px;"></center>
        </div>
  		<center>
  		<table>
  		<tr>
...
<input type='button' id='id02' value='♥' style="color:red;" onclick="setDisable('id02')" />
  ...
  int hit = rs.getInt("Readcount");
	hit++;
  sql = "UPDATE pragmatic SET Readcount=" + hit + " where Ename='" +rs.getString("Ename")+"'"; 
